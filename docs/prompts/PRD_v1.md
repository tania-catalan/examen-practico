```markdown
# Prompt derivado de Especificación – PRD (Product Requirements Document)

> **Propósito**: Guiar la generación de un documento de requisitos de producto (PRD) ligero para el caso FTGO, especificando stakeholders, capacidades de negocio basadas en el libro de Richardson (Cap 2) y requisitos no funcionales (NFRs) trazables al brief del laboratorio.
...
| Título | Generación de PRD (Product Requirements Document) para FTGO |
| Artefacto origen | Brief del Anexo A |
| ID origen | `ANEXO-A` |
...
|--------|-------------|------------------------|
| `E_MISSING_BRIEF` | no se proporcionó el brief | abortar con error |
| `E_INVENTED_DOMAIN` | el output contiene stakeholders o capacidades fuera del brief | rechazar y reintentar |
| `E_INCOMPLETE_NFR` | hay NFRs sin métrica o sin origen | rechazar y reintentar |

## 4. Huecos TODO del prompt PRD
...
- Inventar capacidades o stakeholders fuera del brief (E_INVENTED_DOMAIN).
- Entregar NFRs sin métrica ni origen (E_INCOMPLETE_NFR).
- Generar PRD demasiado extenso que impida el flujo Strangler Fig (big-bang PRD).
- Listar soluciones técnicas en lugar de requisitos de producto (confunde PRD con FSD/ADR).

## Verificación

- Check-1: Cada NFR debe incluir una métrica y referencia exacta al brief (ej. [Brief §A.4 Latencia UX]).
- Check-2: Las 7 capacidades del Cap 2 deben aparecer y tener 1 párrafo de responsabilidad / scope.
- Check-3: Longitud objetivo: ≤ 4 páginas equivalentes (aprox. 1200–1800 palabras).
- Check-4: No aparecen stakeholders externos fuera del brief.

## Examples Input/Outputs

- Input (mínimo): brief Anexo A completo; PRD seed (opcional).
- Output (ejemplo breve): sección NFRs con formato:
  - NFR-01: Latencia UX
    - Métrica: ≤ 200 ms p95
    - Origen: [Brief §A.4 Latencia UX]
    - Justificación: experiencia móvil en pico
- Input (alternativo): PRD parcial → Output: PRD completado y lista de inputs faltantes si algo falta.
