...
- Mezclar nivel 1 y nivel 2 en un mismo diagrama (E_LEVEL_MIXED).
- Omitir System_Ext importantes del brief (Stripe, Google Maps, etc.).
- No indicar tecnología/protocolo en relaciones de nivel 2 (E_NO_TECH_PROTOCOL).
- Crear containers demasiado granulares o irrelevantes para el nivel 2.

## Verificación

- Check-1: Nivel 1 incluye: FTGO (System), ≥1 Person, ≥2 System_Ext.
- Check-2: Nivel 2 incluye ≥5 contenedores y cada relación muestra tecnología/protocolo.
- Check-3: Sintaxis Mermaid válida (prueba rápida de parseo conceptual: bloques `C4Context`/`C4Container` cerrados y etiquetas válidas).
- Check-4: Coherencia con ADRs: si un ADR declara async messaging, aparece un broker en el Container diagram.

## Examples Input/Outputs

- Input: PRD, ADRs, brief Anexo A.
- Output (ejemplo fragmento Mermaid):
  - C4Context: Person(consumer, "Consumidor", "Ordena comida")
  - C4Container: Container(order_service, "Order Service", "Java/Spring Boot", "Toma de pedidos")
  - Rel(order_service, kafka, "Publica OrderCreated", "Kafka")
...
| --- | ---: | --- |
| Sintaxis Mermaid válida | <true/false> | <breve insight / acción recomendada> |
| # contenedores (count) | <valor> | <insight sobre granularidad/coherencia> |
| # relaciones con tecnología/protocolo | <valor> | <insight> |
| Trazabilidad (%) | <valor> | <insight> |
| Reducción de iteraciones (%) | <valor> | <insight> |
| Ediciones humanas (count) | <valor> | <insight> |
| Inventado / Hallucination (%) | <valor> | <insight> |

Nota: añadir una fila por cada modificación relevante (por ejemplo si cambias la decisión de incluir broker). 
